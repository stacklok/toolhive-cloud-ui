  ...
  return (
    ...
    >
      {data.mediaType?.startsWith("image/") && data.url ? (
        ...
        <div
          className="text-muted-foreground flex size-full items-center
            justify-center"
        >
...
export const PromptInput = ({
    ...
    (files: File[] | FileList) => {
      ...
      setItems((prev) => {
        ...
        for (const file of capped) {
          ...
          // Convert to base64 immediately for IPC compatibility
          const reader = new FileReader();
          reader.onload = () => {
            const base64Url = reader.result as string;
            setItems((prev) =>
              prev.map((item) =>
                item.id === fileId ? { ...item, url: base64Url } : item,
              ),
            );
          };
  ...
  return (
    <AttachmentsContext.Provider value={ctx}>
      ...
      <form
        className={cn(
          `bg-background w-full divide-y overflow-hidden rounded-xl border
          shadow-sm`,
          className,
        )}
...
export const PromptInputModelSelectTrigger = ({
  ...
  <SelectTrigger
    className={cn(
      `text-muted-foreground border-none bg-transparent font-medium shadow-none
      transition-colors`,
      `hover:bg-accent hover:text-foreground [&[aria-expanded="true"]]:bg-accent
